// <copyright file="fill-timesheet.tsx" company="Microsoft">
...
interface IFillTimesheetState {
    ...
    selectedWeekdaysOnCalendar: ITimesheetRequest[],
...
class FillTimesheet extends React.Component<IFillTimesheetProps, IFillTimesheetState> {
    ...
    private getSelectedDatesToDuplicateEfforts() {
        if (this.state.selectedWeekdaysOnCalendar && this.state.selectedWeekdaysOnCalendar.length > 0) {
            let selectedDates = this.state.selectedWeekdaysOnCalendar.map((selectedWeekday: ITimesheetRequest) => {
     ...
     */
    private async onEffortsDuplicated(selectedWeekdaysToDuplicate: ITimesheetRequest[]) {
        if (selectedWeekdaysToDuplicate && selectedWeekdaysToDuplicate.length) {
            ...
            let targetDates = selectedWeekdaysToDuplicate.map((userTimesheet: ITimesheetRequest) => userTimesheet.date);
     ...
     */
    private onCalendarActiveDateChange(previousSelectedDate: Date, selectedDate: ITimesheetRequest, isTimesheetFrozen: boolean) {
     ...
     */
    private onSelectedDatesChange(selectedWeekdays: ITimesheetRequest[]) {
