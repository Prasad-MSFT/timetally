// <copyright file="request-review.tsx" company="Microsoft">
...
import ITimesheetRequest from "../../models/timesheet-request";
import { cloneDeep } from "lodash";
import { approveTimesheetsAsync, rejectTimesheetsAsync, getUserTimesheetsAsync } from "../../api/timesheet";
import { getProjectsDetailAsync } from "../../api/project";
...
interface IRequestReviewState {
    ...
    selectedDateInCalendar: ITimesheetRequest,
    selectedWeek: number,
...
class RequestReview extends React.Component<IRequestReviewProps, IRequestReviewState> {
    ...
    tableCheckboxColumnDesign = { minWidth: "10vw", maxWidth: "10vw" }
    ...
    constructor(props: any) {
        ...
        this.state = {
            ...
            isMobileView: this.params.isMobileView === "true" ? true : false,
     ...
     * @param selectedDateInCalendar Selected date in calendar.
    ...
    getUserTimesheetDetailsAsync = async (selectedWeek: number, userId: string) => {
        ...
        this.setState({ isLoading: true });
        let response = await getProjectsDetailAsync(startOfWeekDate, endOfWeekDate, userId, this.handleTokenAccessFailure);
        if (response.status === StatusCodes.OK && response.data) {
    ...
    */
    ...
    getSelectedTimesheetRequests = () => {
        if (this.state.isViewTimesheet && this.state.activeTabIndex === TimesheetTabIndex) {
            return this.state.submittedRequests.filter((submittedRequest: ISubmittedRequest) =>
                moment(submittedRequest.timesheetDate).startOf('day').toDate().valueOf() === moment(this.state.selectedDateInCalendar.date).startOf('day').toDate().valueOf()) as ISubmittedRequest[];
        }
     ...
     */
    onCalendarActiveDateChange = (previousSelectedDate: Date, selectedDate: ITimesheetRequest, isFreeze: boolean) => {
        this.setState({ selectedDateInCalendar: selectedDate });
    }
    ...
    onRejectFromTimesheetClick = () => {
        if (this.state.selectedDateInCalendar.status === TimesheetStatus.Submitted || this.state.selectedDateInCalendar.status === TimesheetStatus.Approved) {
            this.setState((prevState: IRequestReviewState) => ({
                isRejectClick: !prevState.isRejectClick
            }));
        }
    ...
    renderProjectDetails = () => {
        let selectedDateTimesheets = this.state.userTimesheet.filter((userTimesheet: IUserTimesheet) =>
            moment(userTimesheet.timesheetDate).toDate().toDateString() === this.state.selectedDateInCalendar.date.toDateString());
        return (
        ...
        if ((selectedTimesheetRequests && selectedTimesheetRequests.length > 0) || (this.state.isViewTimesheet && this.state.activeTabIndex === TimesheetTabIndex)) {
            return (
                <Flex column gap="gap.medium">
                    ...
                    {((this.state.isViewTimesheet && this.state.activeTabIndex === TimesheetTabIndex) || (selectedTimesheetRequests && selectedTimesheetRequests.length === 1)) && <Text content={`${this.localize("timesheetRejectLabel")} ${(this.state.isViewTimesheet && this.state.activeTabIndex === TimesheetTabIndex) ? moment(this.state.selectedDateInCalendar.date).format("DD/MM/YYYY") : this.formatDate(selectedTimesheetRequests[0])}`} />}
                    ...
                    <Flex.Item push >
                        <Flex gap="gap.small">
                            {!this.state.isRejectClick && (this.state.activeTabIndex === TimesheetTabIndex || this.state.isViewTimesheet) &&
                                <>
                                    <Button disabled={this.state.selectedDateInCalendar.status !== TimesheetStatus.Submitted} className={this.state.isMobileView ? "mobile-reject" : ""} content={this.localize("reject")} onClick={this.onRejectFromTimesheetClick} />
                                    <Button disabled={this.state.selectedDateInCalendar.status !== TimesheetStatus.Submitted} className={this.state.isMobileView ? "mobile-approve" : ""} primary content={this.localize("approve")} onClick={this.onApproveClick} />
                                </>
                            }
    ...
    renderCalendarDateInfo = () => {
        let selectedDateTimesheets = this.state.selectedDateInCalendar;
        if (selectedDateTimesheets) {
            if (!this.state.isMobileView) {
                return (
                    <Flex column gap="gap.small">
                        <Text content={moment(this.state.selectedDateInCalendar.date).format("MMMM DD, YYYY")} weight="semibold" />
                ...
                return (
                    <Flex column gap="gap.small">
                        <Text className="mobile-title" content={moment(this.state.selectedDateInCalendar.date).format("MMMM DD, YYYY")} weight="semibold" />
                        <Flex vAlign="center">
