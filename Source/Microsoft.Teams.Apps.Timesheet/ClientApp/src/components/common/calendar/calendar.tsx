// <copyright file="calendar.tsx" company="Microsoft">
...
interface ICalendarProps {
    ...
    onEffortsDuplicated: (selectedWeekdaysToDuplicate: ITimesheetRequest[]) => void,
    onCalendarEditModeChange: (isInEditMode: boolean) => void,
    onCalendarActiveDateChange?: (previousSelectedDate: Date, selectedDate: ITimesheetRequest, isFrozen: boolean) => void,
    onWeekChange: (weekNumber: number) => void,
    onSelectedDatesChange?: (selectedWeekdaysToDuplicate: ITimesheetRequest[]) => void
}
...
const Calendar: React.FunctionComponent<ICalendarProps> = props => {
    ...
    const [calendarDates, setCalendarDates] = React.useState([] as ITimesheetRequest[]);
    const [selectedWeekdaysToDuplicate, setSelectedWeekdaysToDuplicate] = React.useState([] as ITimesheetRequest[]);
    ...
    function getCalendarDates() {
        ...
        let calendarDates: ITimesheetRequest[] = [];
        ...
        let selectedDateTimesheet = calendarDates.find((timesheet: ITimesheetRequest) =>
     ...
     */
    function getDuplicateHoursSelectionClass(timesheet: ITimesheetRequest) {
        ...
        if (canEffortsBeDuplicated(timesheet)) {
            ...
            let weekdayDetails = selectedWeekdaysToDuplicate.find((weekdayDetails: ITimesheetRequest) => weekdayDetails.date.valueOf() === timesheet.date.valueOf());
     ...
     */
    function canEffortsBeDuplicated(timesheet: ITimesheetRequest) {
     ...
     */
    function onCalendarDateSelected(weekday: ITimesheetRequest) {
        if (isDuplicateHoursActivated) {
            if (!canEffortsBeDuplicated(weekday)) {
                return;
            }

            let weekdayDetails = selectedWeekdaysToDuplicate.find((weekdayDetails: ITimesheetRequest) => weekdayDetails.date.valueOf() === weekday.date.valueOf());
            let selectedWeekdays: ITimesheetRequest[] = selectedWeekdaysToDuplicate ? cloneDeep(selectedWeekdaysToDuplicate) : [];

            if (!weekdayDetails) {
                selectedWeekdays.push(weekday);
            }
            else {
                selectedWeekdays = selectedWeekdays.filter((day: ITimesheetRequest) => day.date.valueOf() !== weekday.date.valueOf());
            }
    ...
    function renderDays() {
        ...
        let weekdays: ITimesheetRequest[] = calendarDates ?? [];

        let weekdayElements: JSX.Element[] = weekdays.map((weekday: ITimesheetRequest, index: number) => {
